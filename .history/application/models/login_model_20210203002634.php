...
class Login_model extends CI_Model
{
    ...
    public function retrieve(
        ...
        // untuk cek login orangtua
        $result_ortu = $this->db->where([
        ...
        if($result_ortu) {
            $this->db->where([
                'siswa_id' => $result_ortu->id,
            ]);
            
            $this->session->set_userdata(['login_level' => 'ortu']);
            return array_merge($this->db->get('login', 1)->row_array(), ['login_level' => 'ortu']);
        }
        // end pengecekan ortu
