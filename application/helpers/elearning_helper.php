...
function default_parser_item($add_item = array())
{
    ...
    $return = array(
        ...
        'copyright_setup'   => 'Copyright &copy; 2020 - ' . date('Y') . ' SMK DB 1 - <a href="http://www.SMK DB 1.sch.id">SAROLANGUN</a>',
    ...
    if ($CI->uri->segment(1) != 'setup') {
        $return['copyright'] = 'Copyright &copy; 2020 - ' . date('Y') . ' ' . get_pengaturan('nama-sekolah', 'value').' </a>';
...
function is_ortu()
{
    ...
    if($CI->session->userdata('login_level') == 'ortu') {
        return true;
    }
