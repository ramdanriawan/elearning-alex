...
class MY_Controller extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        $this->update_link        = 'http://www.SMK DB 1.sch.id/category/new-elearning/feed/';
        $this->portal_update_link = 'http://www.SMK DB 1.sch.id/category/new-elearning/';
        $this->bug_tracker_link   = 'http://www.SMK DB 1.sch.id/category/bug-tracker-new-elearning/';

        # load helper
        $this->load->helper(array('url', 'form', 'text', 'elearning', 'security', 'file', 'number', 'date', 'download', 'plugins'));

        $error_install = "Install aplikasi e-learning by SMK DB 1 (www.SMK DB 1.sch.id): " . anchor('setup', "&rarr; Halaman Install");
        try {
            check_db_connection();
        } catch (Exception $e) {
    ...
    /**
     * Method untuk mengambil informasi yang urgent dari SMK DB 1.sch.id
     *
    ...
    function check_urgent_info($skip_check_time = false)
    {
        ...
        if ($ok_check) {
            $url_urgent_info = "http://elearningupdates.SMK DB 1.sch.id/urgent_info.php";
